...
VIET_CHAR_RE = re.compile(r"[À-ỹà-ỹĐđ]")
        ...
        return [
            {
                ...
                "content": (
                    ...
                    "Translate every natural-language segment (including headings, italic text, and parenthetical notes). "
                    "Do not keep Vietnamese source text unless it is a proper noun or code-like token. "
...
class OpenAITranslator(BaseTranslator):
    ...
    def do_translate(self, text) -> str:
        ...
        # Fallback: if Vietnamese source is still mostly Vietnamese in output,
        # force one more pass (helps italic/parenthetical heading lines).
        is_vi_src = bool(VIET_CHAR_RE.search(text))
        is_vi_out = bool(VIET_CHAR_RE.search(content))
        zh_targets = {"zh", "zh-cn", "zh-tw", "zh-hans", "zh-hant"}
        if is_vi_src and is_vi_out and self.lang_out.lower() in zh_targets:
            retry_messages = [
                {
                    "role": "user",
                    "content": (
                        f"Translate Vietnamese to {self.lang_out}. "
                        "Translate ALL text including italic/parenthetical notes. "
                        "Do not keep Vietnamese words. Keep only placeholders like {v0}.\n\n"
                        f"Source Text: {text}\n\nTranslated Text:"
                    ),
                }
            ]
            ...
            if retry_resp.choices and retry_resp.choices[0].message.content:
                retry_content = retry_resp.choices[0].message.content.strip()
                retry_content = self.think_filter_regex.sub("", retry_content).strip()
                if retry_content:
