...
class TranslateConverter(PDFConverterEx):
    ...
    def receive_layout(self, ltpage: LTPage):
        ...
        def nflag(char: str):
            # Treat natural-language Latin/Vietnamese letters as normal text even in reserved layout blocks
            return bool(char) and bool(re.match(r"[A-Za-zÀ-ỹà-ỹĐđ]", char[0]))
        ...
        for child in ltpage:
            if isinstance(child, LTChar):
                ...
                # 判定当前字符是否属于公式
                if (                                                                                        # 判定当前字符是否属于公式
                    (cls == 0 and not nflag(child.get_text()))                                              # 1. 类别为保留区域（但放行自然语言字母）
